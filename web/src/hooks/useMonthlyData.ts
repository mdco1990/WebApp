...
export function useMonthlyData(ym: YearMonth | null): UseMonthlyDataResult {
  ...
  // mutations
  const autoSaveIncomeSource = useCallback(async (source: IncomeSource, cur: YearMonth) => {
    if (source.id) {
      await updateIncomeSource(source.id, { name: source.name, amount_cents: source.amount_cents })
    } else {
      await createIncomeSource({ name: source.name, year: cur.year, month: cur.month, amount_cents: source.amount_cents })
    }
    await reload()
  }, [reload])

  const autoSaveOutcomeSource = useCallback(async (source: OutcomeSource, cur: YearMonth) => {
    if (source.id) {
      await updateBudgetSource(source.id, { name: source.name, amount_cents: source.amount_cents })
    } else {
      await createBudgetSource({ name: source.name, year: cur.year, month: cur.month, amount_cents: source.amount_cents })
    }
    await reload()
  }, [reload])
  ...
  const deleteOutcome = useCallback(async (id: number) => {
  ...
  return { data, loading, reload, addDefaultData, autoSaveIncomeSource, autoSaveOutcomeSource, deleteIncome, deleteOutcome }
}
