...
// API request options
export type UseApiOptions<TBody = never> = {
  url: string;
...
// Default options
const defaultOptions: Required<UseApiOptions<any>> = {
  url: '',
  method: 'GET',
  body: undefined,
  headers: {},
  ...
  transformError: (error) => error?.message || 'An error occurred',
};

// Main useApi hook
export function useApi<TData = any, TBody = never>(
  initialOptions: UseApiOptions<TBody> = {}
  ...
  // Generate cache key
  const generateCacheKey = useCallback((url: string, method: string, body?: any) => {
  ...
  // Get from cache
  const getFromCache = useCallback((key: string): TData | null => {
  ...
  const makeRequest = useCallback(async (
    requestOptions: UseApiOptions<TBody>
  ): Promise<TData> => {
    const {
      url,
      method,
  ...
  }, [options.url, options.method, executeRequest]);
...
export function useGet<TData = any>(url: string, options?: Partial<UseApiOptions>) {
  return useApi<TData>({ ...options, url, method: 'GET' });
}

export function usePost<TData = any, TBody = any>(url: string, options?: Partial<UseApiOptions<TBody>>) {
  return useApi<TData, TBody>({ ...options, url, method: 'POST' });
}

export function usePut<TData = any, TBody = any>(url: string, options?: Partial<UseApiOptions<TBody>>) {
  return useApi<TData, TBody>({ ...options, url, method: 'PUT' });
}

export function useDelete<TData = any>(url: string, options?: Partial<UseApiOptions>) {
  return useApi<TData>({ ...options, url, method: 'DELETE' });
}

export function usePatch<TData = any, TBody = any>(url: string, options?: Partial<UseApiOptions<TBody>>) {
  return useApi<TData, TBody>({ ...options, url, method: 'PATCH' });
}

// Hook for optimistic updates
export function useOptimisticApi<TData = any, TBody = never>(
...
export function useInfiniteApi<TData = any, TBody = never>(
  options: UseApiOptions<TBody> & {
  ...
  return {
    ...
    hasNextPage: options.getNextPageParam ? 
      options.getNextPageParam(pages[pages.length - 1] || null, pages) !== undefined : 
      false,
    hasPreviousPage: options.getPreviousPageParam ? 
      options.getPreviousPageParam(pages[0] || null, pages) !== undefined : 
      false,
  };
}