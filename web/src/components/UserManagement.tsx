...
const UserManagement: React.FC<UserManagementProps> = ({ onBackToMain }) => {
  ...
  const loadUserData = useCallback(async () => {
    try {
      setLoading(true);
      const [pendingResponse, allResponse] = await Promise.all([ApiService.getPendingUsers(), ApiService.getAllUsers()]);
      setPendingUsers((pendingResponse.data as PendingUser[]) || []);
      setAllUsers((allResponse.data as AdminUser[]) || []);
    } catch {
