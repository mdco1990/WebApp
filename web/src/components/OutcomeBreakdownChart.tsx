...
const OutcomeBreakdownChart = React.memo<Props>(({ isDarkMode, items, formatCurrency }) => {
  const labels = React.useMemo(() => items.map((s) => s.name), [items]);
  const values = React.useMemo(() => items.map((s) => s.amount_cents / 100), [items]);

  const paletteBg = PALETTE_BG;
  const paletteBorder = PALETTE_BORDER;
  ...
  const data = React.useMemo(
    ...
    [labels, values, paletteBg, paletteBorder]
  );
...
OutcomeBreakdownChart.displayName = 'OutcomeBreakdownChart';
