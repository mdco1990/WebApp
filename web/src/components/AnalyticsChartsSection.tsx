...
const AnalyticsChartsSection = React.memo<AnalyticsChartsSectionProps>(({
  ...
  const budgetOverviewLabels = React.useMemo(() => [t('label.totalIncomes'), t('label.totalOutcomes')], [t]);
  const budgetOverviewLegend = React.useMemo(() => t('legend.amount', { defaultValue: 'Amount' }), [t]);
  const savingsLegendCurrent = React.useMemo(() => t('legend.current', { defaultValue: 'Current' }), [t]);
  const savingsLegendRemaining = React.useMemo(() => t('legend.remaining', { defaultValue: 'Remaining' }), [t]);
  const kpiLegendIncome = React.useMemo(() => t('legend.predictedIncome', { defaultValue: 'Predicted Income' }), [t]);
  const kpiLegendOutcome = React.useMemo(() => t('legend.predictedOutcomes', { defaultValue: 'Predicted Outcomes' }), [t]);
  const dailyLegendBank = React.useMemo(() => t('legend.bank', { defaultValue: 'Bank' }), [t]);
  const dailyLegendItems = React.useMemo(() => t('legend.itemsTotal', { defaultValue: 'Items Total' }), [t]);
  const dailyLegendRemaining = React.useMemo(() => t('legend.remaining', { defaultValue: 'Remaining' }), [t]);
  ...
  const kpiTrendTitle = React.useMemo(() => t('section.kpiTrend', { defaultValue: 'Income vs Outcomes Trend' }), [t]);
  const manualBudgetDailyTitle = React.useMemo(() => t('section.manualBudgetDaily', { defaultValue: 'Manual Budget (Daily Series)' }), [t]);
  const noteDailySeries = React.useMemo(() => t('label.noteDailySeries') ?? 'Note: series are plotted per day. Items are planned totals (no dates), so lines are flat unless values change.', [t]);

  // Memoize chart data arrays
  const budgetOverviewData = React.useMemo(() => [predictedBudget.totalIncome / 100, predictedBudget.totalOutcome / 100], [predictedBudget.totalIncome, predictedBudget.totalOutcome]);
  const manualBudgetItemsTotal = React.useMemo(() => manualBudget.items.reduce((s, i) => s + (i.amount || 0), 0), [manualBudget.items]);
              ...
              <div className="col-md-6 col-12">
                ...
                <div className="form-control-plaintext">
                  {(() => {
                    ...
                    const label = i18n.language?.startsWith('fr')
                      ? t('label.months', { defaultValue: 'mois' })
                      : months === 1
                        ? t('label.month', { defaultValue: 'month' })
                        : t('label.months', { defaultValue: 'months' });
                    return `${months} ${label}`;
                  })()}
                </div>
              </div>
