...
func newTestService(t *testing.T) *Service {
	...
	if err := db.Migrate(dbConn); err != nil { t.Fatalf("migrate: %v", err) }
...
func TestSummary(t *testing.T) {
	...
	if err := svc.SetSalary(ctx, ym, 500000); err != nil { t.Fatal(err) }
	if err := svc.SetBudget(ctx, ym, 200000); err != nil { t.Fatal(err) }
	if _, err := svc.AddExpense(ctx, &domain.Expense{YearMonth: ym, Description: "Coffee", AmountCents: 500}); err != nil { t.Fatal(err) }
	sum, err := svc.Summary(ctx, ym)
	if err != nil { t.Fatal(err) }
	if sum.Remaining != 699500 { t.Fatalf("unexpected remaining: %v", sum.Remaining) }
}
